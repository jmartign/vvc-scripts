...
// if unable to do so.
if ((!@include './config.php') || (!@include './functions.php') || (!@include './createntlm.php')) {
?>
<html>
<head>
...
if ($RESETFREQUENCY > 0) {
	if (!($last_change_day = get_value($ds, $dn, 'shadowLastChange'))) {
           // User probably never changed the password
           $last_change_day[0] = 0;
	}
...
if ($SAMBASUPPORT == 1) {
  $lmpass = lmPassword($new_password);
  if (!(@ldap_mod_replace($ds, $dn, array('sambaLMPassword' => $lmpass)))) {
        fatal_error("Unable to change LM password.");
  }
  $ntpass = ntPassword($new_password);
  if (!(@ldap_mod_replace($ds, $dn, array('sambaNTPassword' => $ntpass)))) {
        fatal_error("Unable to change NT password.");
  }
}

// Change the last change date.
if ($CHANGELASTDATE == 1) {
	if (!(@ldap_mod_replace($ds, $dn, array('shadowLastChange' => $current_day)))) {
		$error = "Unable to change last change date";
	}
        if ($SAMBASUPPORT == 1) {
          if (!(@ldap_mod_replace($ds, $dn, array('sambaPwdLastSet' => time())))) {
                $error = "Unable to change samba last change date";
          }
        }
}
...
if (mail($mail_to, $RESET_MAILSUBJECT, $mail_body, "From: $RESET_MAILFROM\r\n" . "Reply-To: $RESET_MAILFROM")) {
	$user = "";
	display_template($TEMPLATE_RESETPASS, "", "Your password has been successfully changed and sent to your email address(es) on file");
}
